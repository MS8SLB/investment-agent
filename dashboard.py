...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    page_header("ASK AGENT — DIRECT DIALOGUE")

    st.markdown(
        '<div style="color:#505050;font-size:12px;line-height:2;margin-bottom:24px;">'
        'Ask the agent anything about the portfolio, a stock, the market, or upload a file for analysis.<br>'
        'The agent has full access to all portfolio tools and live market data.'
        '</div>',
        unsafe_allow_html=True,
    )

    if "ask_agent_running" not in st.session_state:
        st.session_state.ask_agent_running = False
    if "ask_agent_history" not in st.session_state:
        st.session_state.ask_agent_history = []  # list of {question, output_lines}

    section("YOUR QUESTION")

    question = st.text_area(
        "Question",
        placeholder="e.g. What is GEN's current margin of safety? / Analyse the attached annual report / Compare NU vs StoneCo",
        height=100,
        label_visibility="collapsed",
        disabled=st.session_state.ask_agent_running,
    )

    section("ATTACH FILES  (OPTIONAL)")
    ...
    send_btn = st.button(
        "▶  SEND",
        disabled=st.session_state.ask_agent_running or not question.strip(),
    )

    output_area = st.empty()
    status_box  = st.empty()

    if st.session_state.ask_agent_running:
        status_box.markdown(
            '<span style="color:#FF8000;font-size:12px;letter-spacing:1px;">● AGENT THINKING — PLEASE WAIT...</span>',
            unsafe_allow_html=True,
        )
    ...
    if send_btn and question.strip():
        ...
        st.session_state.ask_agent_running = True
        status_box.markdown(
            '<span style="color:#FF8000;font-size:12px;letter-spacing:1px;">● AGENT THINKING — PLEASE WAIT...</span>',
            unsafe_allow_html=True,
        )
            ...
            if ext in IMAGE_TYPES:
                ...
                try:
                    ...
                    extra_text_parts.append(
                        f"\n\n--- Attached file: {html.escape(f.name)} ---\n{content}\n--- End of file ---"
                    )
                except Exception:
        ...
        # Build initial_content: images first, then the text prompt
        initial_content = None
        if image_blocks:
            initial_content = image_blocks + [{"type": "text", "text": full_prompt}]

        # ── Stream output ─────────────────────────────────────────────────────
        lines = [
            '<span style="color:#505050">━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</span>',
            f'<span style="color:#505050">ASK AGENT  {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</span>',
            f'<span style="color:#FF8000">Q: {html.escape(question.strip()[:120])}{"…" if len(question.strip()) > 120 else ""}</span>',
            '<span style="color:#505050">━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</span>',
            "",
        ]
        ...
        def on_tool_call(name: str, inp: dict):
            extra = ""
            for key in ("ticker", "tickers", "query", "mode"):
                val = inp.get(key)
                if val:
                    extra = f'  <span style="color:#505050">{html.escape(str(val)[:60])}</span>'
                    break
            lines.append(f'<span style="color:#FF8000">  ⚙ {html.escape(name)}</span>{extra}')
            refresh()
        ...
        try:
            ...
            lines.append(f'<span style="color:#FF3B3B">ERROR: {html.escape(str(exc))}</span>')
            refresh()

        # Save to in-session history
        st.session_state.ask_agent_history.insert(0, {
        ...
        st.session_state.ask_agent_running = False
        status_box.markdown(
            '<span style="color:#00E676;font-size:12px;letter-spacing:1px;">✓ DONE</span>',
            unsafe_allow_html=True,
        )

    # ── Session history (most recent first) ──────────────────────────────────
    history = st.session_state.get("ask_agent_history", [])
    if not send_btn and history:
        section("PREVIOUS QUESTIONS THIS SESSION")
        for i, entry in enumerate(history):
            label = f"◈  {entry['question'][:80]}{'…' if len(entry['question']) > 80 else ''}"
            with st.expander(label, expanded=(i == 0)):
                st.markdown(agent_box(entry["lines"]), unsafe_allow_html=True)
